# Sri Venkata Durga Sudarsan Madhyannapu
 
Associate Professor, Dr. RVR NRI Institute of Technology (Deemed to be University), Pothavarappadu Village, Agiripalli Mandal 521212, Andhra Pradesh, India

## Research Areas

- Control Theory
- Singular Matrix Differential Systems
- Periodic Matrix Systems
- Hybrid Dynamical Systems
- Physics-Informed Neural Networks (PINNs)
- Scientific Machine Learning
- Chaotic Chemical Reaction Networks
- Nonlinear Dynamical Systems
- MATLAB-Based Computational Methods

## Academic Profiles

ORCID:
https://orcid.org/0009-0001-2126-6428

Google Scholar:
https://scholar.google.com/citations?user=8LAGLi4AAAAJ

ResearchGate:
https://www.researchgate.net/profile/Sri-Venkata-Durga-Sudarsan-Madhyannapu

arXiv:
https://arxiv.org/search/?searchtype=author&query=Madhyannapu%2C+S+V+D+S

GitHub:
https://github.com/msvdsudarsan

Zenodo:
https://zenodo.org


## Selected Research Repositories

- ann-singular-matrix-differential-systems
- SBLIPMS-Impulse-KH-Controllability
- extended-melnikov-crn-benchmarks
- Bilinear-Matrix-Periodic-Controllability
- Bilinear-Matrix-Periodic-Observability
- SBMPMS-observability

## Current Research Focus

Current work focuses on structure-preserving computational methods for singular and periodic dynamical systems, with particular emphasis on controllability analysis, hybrid systems, and physics-informed neural network frameworks for scientific machine learning applications.
